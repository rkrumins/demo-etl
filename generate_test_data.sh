#!/bin/bash
# Fixed Data Generator script (generate_test_data.sh)
# Creates consistent CSV files and uploads them to HDFS for Spark lineage demonstration
...
# Ensuring consistent ID types (integers) for joining
...
# Ensuring consistent ID types (integers) for joining and matching IDs with customers and products
...
hdfs dfs -mkdir -p /data/lineage

# Upload files to HDFS
echo "Uploading files to HDFS..."
hdfs dfs -put -f ./data/customers.csv /data/lineage/
hdfs dfs -put -f ./data/products.csv /data/lineage/
hdfs dfs -put -f ./data/orders.csv /data/lineage/

# Verify files in HDFS
echo "Verifying files in HDFS..."
hdfs dfs -ls /data/lineage/

# Show sample contents to verify data format
echo "Sample data from customers.csv:"
hdfs dfs -cat /data/lineage/customers.csv | head -n 3
echo ""

echo "Sample data from products.csv:"
hdfs dfs -cat /data/lineage/products.csv | head -n 3
echo ""

echo "Sample data from orders.csv:"
hdfs dfs -cat /data/lineage/orders.csv | head -n 3
echo ""

echo "=== Test Data Generation Complete ==="
echo "Files available in HDFS at /data/lineage/"
